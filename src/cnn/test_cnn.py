...
class TestSingleLayerConvNN(unittest.TestCase):
    ...
    def test_output(self):
        ...
        #                        dtype=theano.config.floatX))
        #cnn.layer_0.b_c.set_value(numpy.ones((filter_shape[0],),
        #                                    dtype=theano.config.floatX))
        ...
        #cnn.regression.bias.set_value(numpy.cast['float32'](1))

        cnn.regression.theta.set_value(numpy.asarray(self.rng.uniform(low=-1.0, high=1.0,
            size=(3*3*n_filters,1)), dtype=theano.config.floatX))
