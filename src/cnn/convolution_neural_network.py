...
class TwoLayerConvNN(object):
    def __init__(self, rng, x_data, batch_size, n_hidden_units, image_shape, filter_shape,
                 pool_size):
        self.layer_0_input = x_data.reshape(image_shape)
        self.layer_0 = ConvPoolLayer(rng=rng, 
                                     x_data=self.layer_0_input,
                                     ...
                                     filter_shape=filter_shape, 
                                     pool_size=pool_size)
        self.layer_1_input = self.layer_0.output.flatten(2)
        output_size = calculate_output_size(image_shape, 
                                            filter_shape,
                                            pool_size)
        self.layer_1 = HiddenLayer(rng, self.layer_1_input, output_size,
                                   n_hidden_units)
        self.regression = TheanoLeastSquaresRegression(self.layer_1.output.T, 
                                                       n_hidden_units, 
                                                       batch_size)
        self.l1 = abs(self.layer_0.W).sum() + abs(self.layer_1.W).sum() + abs(self.regression.theta).sum()
        self.l2 = (self.layer_0.W ** 2).sum() + (self.layer_1.W ** 2).sum() + (self.regression.theta ** 2).sum()
...
def train_cnn(data_file_name, batch_size=BATCH_SIZE, reg_lambda_1=REG_LAMBDA_1, 
    ...
    train_data, validation_data, test_data = load_images(data_file_name)
    ...
    #err
    means, ranges = get_means_and_ranges(train_data[0].T)
    train_data[0] = normalize_zero_mean(train_data[0].T, means, ranges).T
    test_data[0] = normalize_zero_mean(test_data[0].T, means, ranges).T
    validation_data[0] = normalize_zero_mean(validation_data[0].T, means, ranges).T
    ...
    while (epochs < n_epochs) and (not done_looping):
        ...
        for batch_index in xrange(n_training_batches):
            ...
            if (iteration + 1) % validation_frequency == 0:
                validation_costs = [validate_model(i) for i in xrange(n_validation_batches)]
                validation_cost = numpy.mean(validation_costs)
                costs_validations.append((iteration, validation_cost))
                logging.info('current validation cost at epoch %d iter %d is %f' 
                             % (epochs, iteration, validation_cost))
        ...
        logging.info('epoch % 5d error % 9f' % (epochs, current_cost))
        epochs += 1
